// db/seed.js — Seed-Fixtures fuer einen Tenant (Menue, Staff/Admin-Logins, Demo-Kunde, Coupon, Kampagnen).
// Generisch fuer JEDEN Tenant nutzbar (Parameter statt Hardcoding) - urspruenglich nur fuer TENANT_001,
// jetzt auch fuer einen dedizierten QA_TENANT nutzbar, damit automatisierte Tests NIE echte Kundendaten
// beruehren muessen (Root Cause fuer wiederholte Testdaten-Kontamination in frueheren Sessions).
// Idempotent (ON CONFLICT DO NOTHING / Existenz-Checks) — sicher mehrfach ausfuehrbar.
const { query, migrate, pool } = require('./index');
const { hashPassword, randomToken } = require('../lib/crypto');

async function upsertTenant(tenantId, name, slug) {
  ...
  `, [tenantId, name, slug, 'Markt 12', '42477', 'Radevormwald',
      '+49 2195 677099', 'am-matt@vodafone.de', '#1B4D3E', '#C9A24B']);
  console.log(`Tenant ${tenantId} (${name}) angelegt.`);
}

async function seedOpeningHours(tenantId) {
  const { rows } = await query(`SELECT COUNT(*) as n FROM opening_hours WHERE tenant_id = $1`, [tenantId]);
  if (Number(rows[0].n) > 0) return;
  for (let wd = 1; wd <= 6; wd++) {
    await query(`INSERT INTO opening_hours (tenant_id, weekday, is_closed, open_time, close_time, slot_order)
      VALUES ($1, $2, 0, '11:30', '14:30', 0)`, [tenantId, wd]);
    await query(`INSERT INTO opening_hours (tenant_id, weekday, is_closed, open_time, close_time, slot_order)
      VALUES ($1, $2, 0, '17:00', '22:30', 1)`, [tenantId, wd]);
  }
  await query(`INSERT INTO opening_hours (tenant_id, weekday, is_closed, open_time, close_time, slot_order)
    VALUES ($1, 0, 1, NULL, NULL, 0)`, [tenantId]);
  console.log(`Öffnungszeiten fuer ${tenantId} geseedet.`);
}
...
async function seedMenu(tenantId) {
  ...
  const { rows } = await query(`SELECT COUNT(*) as n FROM menu_categories WHERE tenant_id = $1`, [tenantId]);
  if (Number(rows[0].n) > 0) {
    for (const [name, url] of Object.entries(dishImages)) {
      await query(`UPDATE menu_items SET image_url = $1 WHERE tenant_id = $2 AND name = $3 AND image_url IS NULL`, [url, tenantId, name]);
    }
    return;
  }
  ...
  for (let catIdx = 0; catIdx < categories.length; catIdx++) {
    ...
    const catInsert = await query(`INSERT INTO menu_categories (tenant_id, name, sort_order) VALUES ($1, $2, $3) RETURNING id`, [tenantId, cat.name, catIdx]);
    ...
    for (let itemIdx = 0; itemIdx < cat.items.length; itemIdx++) {
      ...
      await query(`
        ...
        VALUES ($1, $2, $3, $4, $5, $6, 0, $7, 'am-matt.com/menue Live-Scan', '2026-08-31', $8, 'verified')
      `, [tenantId, catId, name, description, price, vegetarian, imageUrl, itemIdx]);
    }
  }
  console.log(`Speisekarte fuer ${tenantId} geseedet.`);
}

async function seedDemoCustomer(tenantId, email, displayName) {
  const { rows } = await query(`SELECT id FROM customers WHERE tenant_id = $1 AND email = $2`, [tenantId, email]);
  if (rows[0]) {
    await query(`UPDATE customers SET display_name = $2 WHERE id = $1 AND display_name != $2`, [rows[0].id, displayName]);
    return;
  }
  ...
  const insert = await query(`
    ...
    VALUES ($1, $2, $3, $4, $5, '1990-05-15', $6, 0) RETURNING id
  `, [tenantId, email, displayName, hash, salt, qrToken]);
  const customerId = insert.rows[0].id;
  await query(`
    INSERT INTO loyalty_ledger (tenant_id, customer_id, value, reason, source, actor)
    VALUES ($1, $2, 540, 'demo_seed_purchase_history', 'system', 'seed_script')
  `, [tenantId, customerId]);
  await query(`UPDATE customers SET points_balance = 540 WHERE id = $1`, [customerId]);
  console.log(`Demo-Kunde fuer ${tenantId} mit 540 Demo-Punkten angelegt.`);
}

async function seedDemoReward(tenantId) {
  const { rows } = await query(`SELECT COUNT(*) as n FROM rewards WHERE tenant_id = $1`, [tenantId]);
  if (Number(rows[0].n) > 0) return;
  await query(`
    INSERT INTO rewards (tenant_id, title, description, points_cost, active, image_url)
    VALUES ($1, 'Espresso oder Kaffee', 'Ein Heißgetränk nach Wahl auf uns.', 50, 1, '/assets/img/reward-espresso.jpg')
  `, [tenantId]);
  await query(`
    INSERT INTO rewards (tenant_id, title, description, points_cost, active, image_url)
    VALUES ($1, 'Dessert des Hauses', 'Baklava, Eis oder Pfannkuchen — deine Wahl.', 100, 1, '/assets/img/dish-baklava.jpg')
  `, [tenantId]);
  await query(`
    INSERT INTO rewards (tenant_id, title, description, points_cost, active, image_url)
    VALUES ($1, 'Hauptgericht gratis', 'Ein Hauptgericht deiner Wahl bei deinem nächsten Besuch.', 600, 1, '/assets/img/dish-rumpsteak.jpg')
  `, [tenantId]);
  console.log(`Prämien fuer ${tenantId} angelegt.`);
}

async function seedStaffAndAdmin(tenantId) {
  const staffRes = await query(`SELECT id FROM staff_users WHERE tenant_id = $1 AND username = 'personal'`, [tenantId]);
  if (!staffRes.rows[0]) {
    const { hash, salt } = hashPassword('personal1234');
    await query(`INSERT INTO staff_users (tenant_id, username, password_hash, password_salt, role) VALUES ($1, 'personal', $2, $3, 'staff')`, [tenantId, hash, salt]);
    console.log(`Staff-Login fuer ${tenantId} angelegt: personal / personal1234`);
  }
  const adminRes = await query(`SELECT id FROM staff_users WHERE tenant_id = $1 AND username = 'admin'`, [tenantId]);
  if (!adminRes.rows[0]) {
    const { hash, salt } = hashPassword('admin1234');
    await query(`INSERT INTO staff_users (tenant_id, username, password_hash, password_salt, role) VALUES ($1, 'admin', $2, $3, 'admin')`, [tenantId, hash, salt]);
    console.log(`Admin-Login fuer ${tenantId} angelegt: admin / admin1234`);
  }
}

async function seedDemoCoupon(tenantId) {
  const { rows } = await query(`SELECT id FROM coupons WHERE tenant_id = $1 AND code = 'WILLKOMMEN10'`, [tenantId]);
  if (rows[0]) return;
  const now = new Date();
  ...
  await query(`
    ...
    VALUES ($1, 'WILLKOMMEN10', $2, $3, $4, 'percent', 10, $5, $6, 'all', 1, 'live')
  `, [tenantId, '10% auf deine Rechnung',
      'Als Dankeschön für deine Treue: 10% Rabatt auf deinen nächsten Besuch.',
      '/assets/img/dish-schnitzel.jpg', now, in60days]);
  console.log(`Coupon WILLKOMMEN10 fuer ${tenantId} angelegt.`);
}

async function seedDemoCampaign(tenantId) {
  const { rows } = await query(`SELECT COUNT(*) as n FROM campaigns WHERE tenant_id = $1`, [tenantId]);
  if (Number(rows[0].n) > 0) return;
  const now = new Date();
  ...
  await query(`
    ...
    VALUES ($1, $2, $3, $4, 'weekly', 'all', $5, $6, 0, 'app', 'live', 'seed_script')
  `, [tenantId, 'Mittagsangebot der Woche',
  ...
  await query(`
    INSERT INTO campaigns (tenant_id, title, description, image_url, campaign_type, target_segment, start_at, end_at,
      points_bonus, visibility, status, created_by)
    VALUES ($1, $2, $3, $4, 'seasonal', 'all', $5, $6, 0, 'app', 'live', 'seed_script')
  `, [tenantId, 'Spargelzeit im Am-Matt',
  ...
  console.log(`Kampagnen fuer ${tenantId} angelegt.`);
}

// Seedet einen beliebigen Tenant komplett (Fixtures identisch zu TENANT_001, aber isoliert).
async function seedTenant(tenantId, name = 'QA Test-Restaurant', slug = 'qa-test', email = 'demo@am-matt.example', displayName = 'Anna Schmitz') {
  await upsertTenant(tenantId, name, slug);
  await seedOpeningHours(tenantId);
  await seedMenu(tenantId);
  await seedDemoCustomer(tenantId, email, displayName);
  await seedDemoReward(tenantId);
  await seedStaffAndAdmin(tenantId);
  await seedDemoCoupon(tenantId);
  await seedDemoCampaign(tenantId);
}

async function main(closePool = true) {
  await migrate();
  await seedTenant('TENANT_001', 'Restaurant Am-Matt', 'am-matt', 'demo@am-matt.example', 'Anna Schmitz');
  console.log('\n--- SEED ABGESCHLOSSEN ---');
...
module.exports = { main, seedTenant };
